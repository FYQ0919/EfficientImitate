...
class CurlingSimuOneEndEnv(object):
    ...
    def __init__(self, render=False):
        ...
        self.vir_sim = curling_sim.Simulation()
        ## 当局先手队
    ...
    def reset_simu(self, first_player: Player = Player.A, render=None, bgr_img=None, reset_from=None):
        """初始化比赛/重置比赛
        first_player: 此局(End)先手队员,默认为Player.A
        render: None表示维持当前reder状态, True/False表示打开/关闭render
        """
        # assert bgr_img==None and reset_from==None, "reset_simu 函数目前尚未实现这两个参数的处理逻辑, 可能有bug"
        ## 维护render
        ...
        if reset_from is not None:
            ...
            A_coordinates, B_coordinates, turn, curr_player = reset_from
            ## 壶的位置
            for i in range(len(A_coordinates)):
                self.stones_pos[i * 2] = A_coordinates[i]
            for i in range(len(B_coordinates)):
                self.stones_pos[i * 2 + 1] = B_coordinates[i]
            ## 轮数
            self.curr_shot = turn * 2 + curr_player
            ## 初始化队员信息
            self.first_player = first_player
            if curr_player:
                self.curr_player = Player.B
            else:
                self.curr_player = Player.A
            ## 其它壶并排放置
            ...
            # 其他壶放置并排放置
            for i in range(0, int(n_curling_stones / 2)):
                self.stones_pos[i * 2] = [0.70, 0.4 + i * 0.50]
                self.stones_pos[i * 2 + 1] = [0.15, 0.4 + i * 0.50]
            ## 设置冰壶的有效性状态
            # self.stones_valid[0] = STATE_VALID
            # self.stones_valid[1] = STATE_VALID
            self.stones_valid[0:] = STATE_READY
        else:
            raise NotImplementedError()
            ## 壶的位置
    ...
    def reset_vir_simu(self, first_player: Player = Player.A, render=False):
        """初始化比赛/重置比赛
        first_player: 此局(End)先手队员,默认为Player.A
        render: None表示维持当前reder状态, True/False表示打开/关闭render
        """
    ...
    def vir_shot_vec(self,
                     vec: list or np.ndarray or torch.tensor,
                     player: Player or int = None):
        """用于解析上层policy输出的action vector"""
        return self.vir_shot(pos_x=vec[0],
                             pos_y=vec[1],
                             w=vec[2],
                             vel=vec[3:5],
                             player=player)
    ...
    def shot(self,
        ...
        pos_y: float 控制出手位置 y 坐标 [0.0, 1.0]
            出手位置坐标: [l1+l2+r4+pos_x, width*(pos_y)]
        w: float 自旋, 负为逆时针, 正为顺时针 
    ...
    def vir_shot(self,
        ...
        # 冰壶位置
        self.x_np[0] = self.stones_pos
        self.vir_sim.x.from_numpy(self.x_np)
        self.vir_sim.x[0, self.curr_shot] = [l1 + l2 + r4 + pos_x,
                                             width * (pos_y)]
        # 冰壶初速度
        self.vir_sim.v.fill(0.0)
        self.vir_sim.v[0, self.curr_shot] = vel
        # 冰壶自旋
        self.vir_sim.w.fill(0.0)
        self.vir_sim.w[self.curr_shot] = w
        # 其他参数
        self.vir_sim.x_inc.fill(0.0)
        self.vir_sim.impulse.fill(0.0)
        # 冰壶有效性
        self.vir_sim.vas.from_numpy(self.stones_valid)
        self.vir_sim.vas[self.curr_shot] = STATE_VALID
        ## 开始仿真
        self.vir_sim.init_shot_sim()
        res, res_str, end_time, end_pos = self.vir_sim.vir_forward(curr_shot=self.curr_shot, render=False)

        return res, res_str, end_pos
...
if __name__ == '__main__':
    ...
    env.reset_simu(Player.A)
    env.shot(2.3, 0.5, 0.0, [2.0, 0.0], env.curr_player)
    env.shot(2.3, 0.5, 0.0, [3.0, 0.0], env.curr_player)
    pass
