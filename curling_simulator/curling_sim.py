...
class Simulation:
    def __init__(self, ):
        ...
        self.is_protected = ti.field(shape=(n_curling_stones), dtype=ti_int)  # 是否在自由防守区内
        # stone_pair
    ...
    @ti.func
    def in_free_defense_zone(self, i: ti_int, t: ti_int = 0) -> ti_int:
        """判断是否在自由防守区内"""
        return (self.x[t, i][0] > l1 + l2 + l3 + l4) \
            and (self.x[t, i][0] < l1 + l2 + l3 + l4 + l3) \
            and ((self.x[t, i] - center_pos).norm() > r4)
    ...
    def init_shot_sim(self, ):
        ...
        for i in range(n_curling_stones):
            ...
            # 检查每个球是否在自由防守区内
            self.is_protected[i] = self.in_free_defense_zone(i)
    ...
    def time_integrate(self, t: ti.i32):
        for i in range(n_curling_stones):
            ...
            self.theta[i] = ti.atan2(self.v[t - 1, i][1], self.v[t - 1, i][0])
            # 计算 wp
            w_p = (v_total / r_p)
    ...
    def draw_ground(self):
        """绘制冰壶场地"""
        # Tee
        self.gui.circle(((l1 + l2) * scale / gui_length, (width / 2) * scale / gui_width), radius=int(r4 * scale),
                        color=0x000080)
        self.gui.circle(((l1 + l2) * scale / gui_length, (width / 2) * scale / gui_width), radius=int(r3 * scale),
                        color=0xEEEEEE)
        self.gui.circle(((l1 + l2) * scale / gui_length, (width / 2) * scale / gui_width), radius=int(r2 * scale),
                        color=0xDC143C)
        self.gui.circle(((l1 + l2) * scale / gui_length, (width / 2) * scale / gui_width), radius=int(r1 * scale),
                        color=0xEEEEEE)

        self.gui.circle(((l1 + l2 + l3 + l4 + l3) * scale / gui_length, (width / 2) * scale / gui_width),
                        radius=int(r4 * scale), color=0x000080)
        self.gui.circle(((l1 + l2 + l3 + l4 + l3) * scale / gui_length, (width / 2) * scale / gui_width),
                        radius=int(r3 * scale), color=0xEEEEEE)
        self.gui.circle(((l1 + l2 + l3 + l4 + l3) * scale / gui_length, (width / 2) * scale / gui_width),
                        radius=int(r2 * scale), color=0xDC143C)
        self.gui.circle(((l1 + l2 + l3 + l4 + l3) * scale / gui_length, (width / 2) * scale / gui_width),
                        radius=int(r1 * scale), color=0xEEEEEE)

        # Tee line
        self.gui.line(((l1 + l2) * scale / gui_length, 0), ((l1 + l2) * scale / gui_length, width * scale / gui_width),
                      radius=1, color=0xFFB6C1)
        self.gui.line(((l1 + l2 + l3 + l4 + l3) * scale / gui_length, 0),
                      ((l1 + l2 + l3 + l4 + l3) * scale / gui_length, width * scale / gui_width), radius=1,
                      color=0xFFB6C1)

        # 后卫线
        self.gui.line(((l1 + l2 - r4) * scale / gui_length, 0),
                      ((l1 + l2 - r4) * scale / gui_length, width * scale / gui_width), radius=1, color=0xFFB6C1)
        self.gui.line(((l1 + l2 + l3 + l4 + l3 + r4) * scale / gui_length, 0),
                      ((l1 + l2 + l3 + l4 + l3 + r4) * scale / gui_length, width * scale / gui_width), radius=1,
                      color=0xFFB6C1)

        # Centre line
        self.gui.line((l1 * scale / gui_length, width / 2 * scale / gui_width),
                      ((length - l1) * scale / gui_length, width / 2 * scale / gui_width), radius=1, color=0xCD5C5C)

        # Hog line
        self.gui.line(((l1 + l2 + l3) * scale / gui_length, 0),
                      ((l1 + l2 + l3) * scale / gui_length, width * scale / gui_width), radius=4, color=0xDC143C)
        self.gui.line(((l1 + l2 + l3 + l4) * scale / gui_length, 0),
                      ((l1 + l2 + l3 + l4) * scale / gui_length, width * scale / gui_width), radius=4, color=0xDC143C)

        # Hack
        self.gui.line((l1 * scale / gui_length, 0.5 - hack_length / 2 * scale / gui_width),
                      (l1 * scale / gui_length, 0.5 + hack_length / 2 * scale / gui_width), radius=5, color=0x000000)
        self.gui.line(((length - l1) * scale / gui_length, 0.5 - hack_length / 2 * scale / gui_width),
                      ((length - l1) * scale / gui_length, 0.5 + hack_length / 2 * scale / gui_width), radius=5,
                      color=0x000000)
    ...
    # remove_oppo_num = ti.field(shape=(), dtype=ti_int)
    remove_num = ti.field(shape=(2), dtype=ti_int) # 双方被移走的自由防守区内球的数量

    @ti.kernel
    def detect_stones(self, t: ti_int):
        """出界检测和静止检测"""
        self.moving_num[None] = 0  # 当前运动的球的数量
        self.remove_num.fill(0) # 双方自由防守区内球被移走的数量
        ...
        for i in range(n_curling_stones):
            ...
            # 如果出界, 即刻移除
            if res == 1:
                self.remove_num[i%2] += (1 if self.is_protected[i] == 1 else 0)
                self.move_away(t, i)
                continue
            # 判断静止
    ...
    @ti.kernel
    def detect_vir_stones(self, t: ti_int):
        """出界检测和静止检测"""
        assert NotImplementedError("detect_vir_stones 函数逻辑未实现 / 可能存在问题"); return None
        self.moving_num[None] = 0  # 当前运动的球的数量
    ...
    def forward(self, curr_shot, render=True, interval=vis_interval, detect=True, wait_time=0.000):  # TODO: 用于看效果时调整速率
        ...
        for t in range(1, max_steps):
            ...
            # 五壶保护下对手有球被移走
            if curr_shot < 5 and self.remove_num[1 - (curr_shot % 2)] > 0:
                print(f"检测到五壶保护内有球被移除, t={t}")
                return False, "FIVE_STONE_PROTECT", t
            ## 检测到所有球静止
            if self.moving_num[None] == 0:
                # print(f"检测到所有球静止, t={t}")
                # print(self.x[t, curr_shot])
                return True, "OK", t
        # 达到 max_steps, 仍有球在运动, 返回 END_NOT_STATIC 代码
        return True, "END_NOT_STATIC", -1,

    def vir_forward(self, curr_shot, render=True, interval=vis_interval, detect=True,
                    wait_time=0.000):  # TODO: 用于看效果时调整速率
        """仿真单个shot, 直到shot结束
        curr_shot: Int  env.curr_shot
        render:    Bool 是否渲染
        interval:  Int  GUI绘制的间隔
        detect:    Bool 是否做出界检测和静止检测
        """
        assert NotImplementedError("vir_forward 函数逻辑未实现 / 可能存在问题"); return None
        for t in range(1, max_steps):
            ## 计算碰撞
            self.time_integrate(t)

            self.detect_vir_stones(t)
            ## 检测到所有球静止
            if self.moving_num[None] == 0:
                # print(f"检测到所有球静止, t={t}")
                final_pos = self.x[t, curr_shot]
                return True, "OK", t, final_pos
        # 达到 max_steps, 仍有球在运动, 返回 END_NOT_STATIC 代码
        final_pos = self.x[t, curr_shot]
        return True, "END_NOT_STATIC", -1, final_pos
